...
export const SectionToolbar = React.forwardRef<
  HTMLDivElement,
  SectionToolbarProps
>(({ onAction, isProcessing, position, sectionTitle }, ref) => {
  ...
  return (
    ...
    >
      ...
      {isProcessing ? (
        ...
        <>
          <ActionButton
            onClick={() => onAction("rewrite_formal")}
            label="Make Formal"
            icon="⚖️"
          />
          <ActionButton
            onClick={() => onAction("rewrite_simple")}
            label="Simplify"
            icon="💡"
          />
          <ActionButton
